...
const conv: ConvItem[] = [
  { cat: 'icebreaker', q: "What's your favorite thing to do on a lazy weekend?" },
  { cat: 'icebreaker', q: 'Do you prefer coffee dates or dinner dates?' },
  { cat: 'icebreaker', q: "What's the most spontaneous thing you've ever done?" },
  { cat: 'icebreaker', q: 'What kind of music sets the mood for a great night?' },
  { cat: 'icebreaker', q: 'What does a typical weekend look like for you?' },
  { cat: 'icebreaker', q: 'If you could take a day trip anywhere tomorrow, where would you go?' },
  { cat: 'deep', q: 'What values are most important to you in a relationship?' },
  { cat: 'deep', q: 'How do you define success?' },
  { cat: 'deep', q: "What makes you feel alive — and what's on your bucket list?" },
  { cat: 'deep', q: "What's something you're proud of?" },
  { cat: 'deep', q: 'What helps you feel loved, safe, and protected?' },
  { cat: 'deep', q: 'What assumption do people make about you that is totally wrong?' },
  { cat: 'deep', q: "What's the most important lesson you've learned in life?" },
  { cat: 'fun', q: 'If you could have dinner with any fictional character, who would it be?' },
  ...
  { cat: 'fun', q: 'Would you rather have an incredibly fast car or incredibly fast internet?' },
];
      ...
      {Array.from({ length: max }, (_, i) => (
        <div
          ...
          style={{
            ...
            background: i === active % max ? 'var(--color-text-primary, #111)' : 'var(--color-border-tertiary, #ddd)',
...
export default function VibeCard() {
  ...
  const filtered = convCat === 'all' ? conv : conv.filter((d) => d.cat === convCat);
  const convItem = filtered[convIdx % filtered.length];
  const faqItem = faqs[faqIdx];

  const convDmHref =
    WA_BASE +
    encodeURIComponent(`Hey! I saw your conversation starter: "${convItem.q}" — I'd love to chat about this.`) +
    '&type=phone_number&app_absent=0';

  const faqDmHref =
    WA_BASE +
    encodeURIComponent(`Hey! I saw your FAQ: "${faqItem.q}" — I'd love to discuss this with you.`) +
    '&type=phone_number&app_absent=0';
        ...
        {(['conv', 'faq'] as Tab[]).map((t, i) => (
          <button
            ...
            style={{
              ...
              background: 'none',
              border: 'none',
              borderBottom: tab === t ? '2px solid var(--color-text-primary, #111)' : '2px solid transparent',
              fontFamily: 'inherit',
              fontSize: 13,
              fontWeight: 500,
            ...
            {cats.map((c) => (
              <button
                ...
                style={{
                  ...
                  background: convCat === c ? 'var(--color-text-primary, #111)' : 'none',
                  color: convCat === c ? 'var(--color-background-primary, #fff)' : 'var(--color-text-secondary, #888)',
                  cursor: 'pointer',
                  transition: 'all 0.15s',
      ...
      {tab === 'faq' && (
        <div>
          ...
          <div
            ...
            onTouchStart={(e) => { faqTouchX.current = e.touches[0].clientX; }}
            onTouchEnd={(e) => {

...
const stats = [
  { value: "7+", label: "Years in Backend", icon: <Terminal className="w-5 h-5" /> },
  { value: "20+", label: "Production Apps", icon: <Globe className="w-5 h-5" /> },
...
const projects = [
  {
    ...
    tag: "HealthTech",
    impact: "Built end-to-end health insurance onboarding serving thousands of users in Nigeria.",
  ...
  },
  {
    name: "Instant OTP",
    tag: "Infrastructure",
    impact: "Multi-tenant white-label SMS verification panel with real-time DNS management.",
    stack: ["Go", "Redis", "AWS", "Nginx"],
  },
  {
    name: "PaperDB",
    tag: "BaaS",
    impact: "Type-safe backend actions directly from frontend. Designed for high-performance ingestion.",
...
const experience = [
  {
    ...
    points: [
      ...
      "Scaling healthtech access for 500+ businesses via Proton.",
    ]
  },
  {
    period: "2020 – 2024",
  ...
  return (
    <div className="min-h-screen bg-[#FAF8F5] text-[#1A1A1A] selection:bg-[#D4A843]/30">
      <style jsx global>{`
        ...
        .border-forest/10 { border-color: rgba(26, 77, 62, 0.1); }
      `}</style>
      ...
      <section className="relative pt-40 pb-20 px-6 overflow-hidden">
        <div className="max-w-4xl mx-auto text-center">
          <div className="relative z-10 flex flex-col items-center">
            ...
            >
              I build backend systems that don&apos;t just work—they scale. With 7+ years of 
              founder-led experience, I bridge the gap between complex architecture 
              and business ROI.
            </motion.p>
            ...
            >
              <a href="mailto:mosesedem81@gmail.com" className="bg-forest text-white px-8 py-4 rounded-full font-bold flex items-center gap-3 hover:gap-5 transition-all shadow-xl shadow-forest/20">
      ...
      <section className="py-32 px-6 bg-white">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid lg:grid-cols-3 gap-8">
            {projects.filter(p => activeFilter === "All" || p.tag === activeFilter).map((project) => (
              <motion.div 
                layout
                key={project.name}
              ...
              >
                <div className="flex justify-between items-start mb-6">
                  ...
                  {project.link && (
                    <a href={project.link} target="_blank" className="text-gray-300 hover:text-forest transition-colors">
                      <ExternalLink className="w-5 h-5" />
                    </a>
                  )}
                </div>
                <h3 className="text-2xl font-serif text-forest mb-4">{project.name}</h3>
                <p className="text-gray-500 text-sm leading-relaxed mb-8">{project.impact}</p>
                <div className="flex flex-wrap gap-2">
                  {project.stack.map(s => (
                    <span key={s} className="text-[10px] font-bold text-forest/60 border border-forest/10 px-2 py-1 rounded-md bg-white">
                      {s}
                    </span>
                  ))}
                </div>
      ...
      <section className="py-32 px-6">
        <div className="max-w-4xl mx-auto">
          <SectionHeader label="Professional Path" title="The Journey of Impact" />
          ...
          <div className="space-y-16">
            {experience.map((item) => (
              ...
              >
                ...
                <span className="text-sm font-bold text-gold mb-2 block">{item.period}</span>
                <h3 className="text-2xl font-serif text-forest mb-1">{item.role}</h3>
                <p className="text-lg font-medium text-gray-400 mb-6">{item.company}</p>
                <ul className="space-y-4">
                  {item.points.map((point, pi) => (
                    <li key={pi} className="flex gap-4 text-gray-500 leading-relaxed italic">
      ...
      <section className="py-32 px-6 bg-forest text-white overflow-hidden relative">
        <div className="max-w-5xl mx-auto text-center relative z-10">
          <motion.div 
            ...
            viewport={{ once: true }}
          >
            <h2 className="text-4xl md:text-6xl font-serif mb-8">Ready to scale your next big idea?</h2>
            <p className="text-xl text-white/60 max-w-2xl mx-auto mb-12">
              Whether you need a full-stack lead, a backend specialist, or a strategic 
              technical consultant, I&apos;m ready to ship.
            </p>
            
            <div className="flex flex-col md:flex-row gap-6 justify-center items-center">
              <a href="mailto:mosesedem81@gmail.com" className="bg-gold text-forest px-12 py-5 rounded-full font-black uppercase tracking-widest text-sm hover:scale-105 transition-all shadow-2xl shadow-gold/20">
                Email Me Directly
              </a>
              <div className="flex gap-6">
                <a href="tel:+2349030465501" className="text-white/60 hover:text-white transition-colors flex items-center gap-2 font-bold">
      ...
      <footer className="py-12 px-6 border-t border-forest/5">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-8">
          <div className="text-sm text-gray-400 italic">
            © {new Date().getFullYear()} Moses Jacob Edem. Engineered for excellence.
          </div>
          <div className="flex gap-6">
            <a href="https://github.com/mosesedem" target="_blank" className="text-forest hover:text-gold transition-colors"><Github className="w-5 h-5" /></a>
            <a href="https://linkedin.com/in/mosesedem" target="_blank" className="text-forest hover:text-gold transition-colors"><Linkedin className="w-5 h-5" /></a>
            <a href="mailto:mosesedem81@gmail.com" className="text-forest hover:text-gold transition-colors"><Mail className="w-5 h-5" /></a>
          </div>
        </div>
      </footer>
    </div>
